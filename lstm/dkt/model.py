...
class LSTM(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cate_proj = nn.Sequential(
            nn.Linear((self.hidden_dim // 3) * (n_categorical - 1), self.hidden_dim), # 1 빼는 이유: categorical에서 userID는 제외대상
            nn.LayerNorm(self.hidden_dim)
        )
        
        # cont embedding
        self.cont_bn = nn.BatchNorm1d(self.cont_count)
        self.cont_proj = nn.Sequential(
            nn.Linear(self.cont_count, self.hidden_dim),
            nn.LayerNorm(self.hidden_dim)
        )
...
class Saint(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.dec_comb_proj = nn.Linear(self.hidden_dim * 2, self.hidden_dim)
    ...
    def forward(self, input):
        ...
        if self.args.is_cont:
            ...
            comb_embed_dec = torch.cat([cate_embed_dec, cont_embed_dec], 2)
